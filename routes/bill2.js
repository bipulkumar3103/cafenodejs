                ...
                totalAmount:orderDetails.totalAmount},(err,results)=>{
                    if(err){
                        ...
                        pdf.create(results , {
                          ...
                          }).toStream('./generated_pdf/'+ generatedUuid+".pdf", function(err,results){
                            if(err){
                                console.log(err);
                                return res.status(500).json(err);
                            }
                ...
                totalAmount:orderDetails.totalAmount},(err,results)=>{
                    if(err){
                        ...
                        pdf.create(results, {
                          ...
                          }).toStream('./generated_pdf/'+ orderDetails.uuid+".pdf", function(err,results){
                            if(err){
    ...
    connection.query(query,[id],(err,results)=>{
        if(!err){
            if(results.affectedRows==0){
                ...
                return res.status(200).json({message:"Bills Deleted Successfully"});
            }
        }
        else{
            return res.status(500).json(err)
